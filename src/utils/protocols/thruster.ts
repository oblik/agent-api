...
export default async (
  ...
  let params: (string | bigint | number | JSONObject)[] = [];
  ...
  while (true) {
    ...
    try {
      if (!fee) {
        ...
        throw new Error(
          getUnsupportedPoolError(
            chainName ?? "thruster chainName?",
            "thruster",
            poolName ?? "thruster poolName",
          ),
        );
      }

      const thrusterFactoryAbi = getABIForProtocol("thruster", "factory");
      const thrusterPairAbi = getABIForProtocol("thruster", "pair");
      ...
      if (!factoryAddress) {
        throw new Error("Could not find factory for Thruster");
      }
      ...
      switch (action) {
        ...
        case "withdraw": {
          ...
          if (range) {
            ...
            if (!address) {
              throw new Error(
                "Could not find position manager for Thruster withdraw",
              );
            }
            abi = getABIForProtocol("thruster", "position-manager");
            funcName = "multicall";
            ...
            if (hasEth) {
              const { data: unwrapWETHCallData } = await getFunctionData(
                ...
                "unwrapWETH9",
                [
                  ((isToken0Eth ? amount0 : amount1) * 99n) / 100n,
                  accountAddress,
                ],
              );
          ...
          if (!address) {
            throw new Error("Could not find router for Thruster withdraw");
          }
          abi = getABIForProtocol("thruster");
          ...
          if (isAllAmount) {
            ...
            if (
              ...
              if (!amount2) {
                if (!reserve0) {
                  throw new Error(
                    "Thruster failed to return swap data for this swap, please try again.",
                  );
                }
          ...
          if (hasEth) {
            ...
            if (!firstParam) {
              throw new Error(
                "Token addresses are required for Thruster withdraw",
              );
            }
            // removeLiquidityETH
...
const getPoolAddress = async (
  ...
  if (!factoryV3Address) {
    throw new Error("Could not find v3Factory for Thruster");
  }
  const factoryV3Abi = getABIForProtocol("thruster", "factory-v3");
...
async function thDeposit(
  ...
  accountAddress: string,
  funcName0: string,
  params: (string | number | bigint | JSONObject)[],
  value0: number | bigint,
  approveTxs0: Transaction[],
  fee: string,
  factoryAddress: string,
  thrusterFactoryAbi: ethers.InterfaceAbi,
  thrusterPairAbi: ethers.InterfaceAbi,
) {
  ...
  try {
    ...
    if (tokenInfo2 && amount2) {
      ...
      if (
        ...
        if (
          ...
          throw new Error(
            `Depositing into Thruster ${poolName} pool is not supported with ${tokenName}. Try depositing ${tokenSymbols[0].toLowerCase()} or ${tokenSymbols[1].toLowerCase()}.`,
          );
        }
      }
    ...
    if (range) {
      ...
      if (!address) {
        throw new Error("Could not find position manager for Thruster deposit");
      }
      abi = getABIForProtocol("thruster", "position-manager");
      const manager = new ethers.Contract(address, abi, provider);
      const WETH = (await manager.WETH9()).toLowerCase();
      const poolV3Abi = getABIForProtocol("thruster", "pool-v3");
      ...
      if (!tokenId) {
        ...
        for (const fee of feeTiers) {
          ...
          if (poolAddr !== NATIVE_TOKEN) {
            feeTier = fee;
            throw new Unwind(Flow.Break, "th/deposit");
          }
        }
        if (!poolAddr) {
    ...
    address = getProtocolAddressForChain("thruster", chainId, `router${fee}`);
    if (!address) {
      throw new Error("Could not find router for Thruster deposit");
    }
    abi = getABIForProtocol("thruster");
    ...
    if (pairAddr === NATIVE_TOKEN) {
      if (fee === "03") throw new Unwind(Flow.Continue, "thruster");
      throw new Error("Pool does not exist");
    }
