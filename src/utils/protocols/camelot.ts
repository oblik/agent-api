...
export default async (
  ...
  const camelotFactoryAbi = getABIForProtocol("camelot", "factory");
  const camelotPairAbi = getABIForProtocol("camelot", "pair");
  const tokenName = actionData.token;

  switch (action) {
    case "claim": {
      address = getProtocolAddressForChain("camelot", chainId, "dividendsV2");
      if (!address) {
        throw new Error("Could not find dividendsV2 for Camelot");
      }
      abi = getABIForProtocol("camelot", "dividendsV2");

      const dividendsV2 = new ethers.Contract(address, abi, provider);

      if (tokenInfo?.address) {
        if (!(await dividendsV2.isDistributedToken(tokenInfo?.address))) {
          throw new Error("Token is not distributed token.");
        }
        if (
          (await dividendsV2.pendingDividendsAmount(
            tokenInfo?.address,
            accountAddress,
          )) === 0n
        ) {
          throw new Error("Pending dividends amount is zero.");
        }

        funcName = "harvestDividends";
        params.push(tokenInfo?.address);
      } else {
        funcName = "harvestAllDividends";
      }
      break;
    }
    case "deposit": {
    ...
    case "withdraw": {
      ...
      if (tokenInfo2 && amount2) {
        ...
        if (
          ...
          if (
            ...
            throw new Error(
              `Withdrawing from Camelot ${poolName} pool is not supported with ${tokenName}. Try withdrawing ${tokenSymbols[0].toLowerCase()} or ${tokenSymbols[1].toLowerCase()}.`,
            );
          }
        }
      ...
      if (range) {
        ...
        if (!address) {
          throw new Error("Could not find position manager for Camelot");
        }
        abi = getABIForProtocol("camelot", "position-manager");
        funcName = "multicall";
      ...
      address = getProtocolAddressForChain("camelot", chainId);
      abi = getABIForProtocol("camelot");
      if (!address) {
        throw new Error("Could not find address for Camelot");
      }
      ...
      if (!factoryAddress) {
        throw new Error("Could not find factory for Camelot");
      }
      ...
      if (isAllAmount) {
        ...
        if (
          ...
          if (!amount2) {
            if (!reserve0) {
              throw new Error(
                "Camelot failed to return swap data for this swap, please try again.",
              );
            }
    ...
    default: {
      throw new Error(
        getUnsupportedActionError(action, ["deposit", "withdraw"], "Camelot"),
      );
    }
  }
...
const getPoolAddress = async (
  ...
  const factoryV3Abi = getABIForProtocol("camelot", "factory-v3");
  const factoryV3 = new ethers.Contract(
    factoryV3Address,
    factoryV3Abi,
    provider,
  );
  return await factoryV3.poolByPair(token0, token1);
};
...
async function clDeposit(
  ...
  camelotFactoryAbi: ethers.InterfaceAbi,
  camelotPairAbi: ethers.InterfaceAbi,
) {
  ...
  try {
    ...
    if (tokenInfo2 && amount2) {
      ...
      if (
        ...
        if (
          ...
          throw new Error(
            `Depositing into Camelot ${poolName} pool is not supported with ${tokenName}. Try depositing ${tokenSymbols[0].toLowerCase()} or ${tokenSymbols[1].toLowerCase()}.`,
          );
        }
      }
      token1 = await getTokenInfoForChain(token1Symbol, chainName, true);
    ...
    if (range) {
      ...
      abi = getABIForProtocol("camelot", "position-manager");
      const manager = new ethers.Contract(address, abi, provider);
      const WETH = (await manager.WNativeToken()).toLowerCase();
      const poolV3Abi = getABIForProtocol("camelot", "pool-v3");
      ...
      if (!tokenId) {
        ...
        const currentTick = Number.parseFloat(
          (await poolContract.globalState())[1].toString(),
        );
      ...
      throw new Unwind(Flow.Break, "cl/deposit");
    }
