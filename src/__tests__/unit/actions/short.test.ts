...
const endpoint = `/short?secret=${process.env.BACKEND_TOKEN_SECRET}`;
...
describe("Short", () => {
  ...
  // unsupported
  it.skip("GMX - Short with Invalid Token Error", async () => {
    const res = await request(app).post(endpoint).send({
      accountAddress: "0xD6216fC19DB775Df9774a6E33526131dA7D19a2c",
      protocolName: "GMX",
      chainName: "Arbitrum",
      inputToken: "invalidToken", // Assume an unsupported token for shorting
      inputAmount: "100",
      outputToken: "WETH",
      leverageMultiplier: 4,
    });
  ...
  // unsupported
  it.skip("GMX - Short with Negative Amount Error", async () => {
    const res = await request(app).post(endpoint).send({
      accountAddress: "0xD6216fC19DB775Df9774a6E33526131dA7D19a2c",
      protocolName: "GMX",
      chainName: "Arbitrum",
      inputToken: "USDT",
      inputAmount: "-50", // Assume a negative shorting amount
      outputToken: "WETH",
      leverageMultiplier: 4,
    });
    expect(res.statusCode).toEqual(400);
  ...
  it.skip("GMX - Unsupported Output Token Error", async () => {
    ...
    expect(res.body).toHaveProperty("message");
    expect(res.body.message).toEqual(
      `shorting dai is not supported on gmx. The available tokens to short are ${availableMarkets(42161)}.`,
    );
  });
  ...
  it("Hyperliquid - Invalid input token", async () => {
    ...
    expect(res.body).toHaveProperty("message");
    expect(res.body.message).toEqual(
      "Token USDT is not supported to short on Hyperliquid. Only USDC is supported.",
    );
  });
