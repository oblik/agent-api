...
function createMatchedEvents(
  ...
  // Create a matched event for each execution
  return matchingExecutedEvents.map((executedEvent) => ({
    userId: submitEvent.properties.distinct_id,
    prompt: submitEvent.properties.Prompt,
    time: submitEvent.properties.time,
    messageId: submitEvent.properties.messageId,
    blocker:
      executedEvent.properties.Status.toLowerCase() === "failed"
        ? "failed"
        : "success",
  }));
}
...
async function main() {
  ...
  try {
    ...
    console.log("Fetching Action Executed Test events...");
    ...
    for (const submitEvent of submitEvents) {
      ...
      if (matchingExecutedEvents.length > 1) {
        console.log(
          `Multiple executed events found for messageId ${messageId}:`,
        );
        console.log("Submit event:", submitEvent);
        console.log("Matching executed events:", matchingExecutedEvents);
      }

      // Create matched events for all executions (or none if no executions)
      const events = createMatchedEvents(submitEvent, matchingExecutedEvents);
      matchedEvents.push(...events);
    }

    matchedEvents.sort((a, b) => b.time - a.time);

    // Generate CSV with messageId column
    const csvHeader = "User ID\tPrompt\tTime\tMessage ID\tBlocker\n";
    const csvRows = matchedEvents
      .map(
        (event) =>
          `${distinctIdToUsername[event.userId]}\t"${event.prompt}"\t${event.time}\t${event.messageId}\t${event.blocker}`,
      )
      .join("\n");
    ...
    // Count submissions with multiple executions
    const multipleExecutionsCount = Array.from(
      executedEventMap.entries(),
    ).filter(([_, events]) => events.length > 1).length;
    console.log(
      `\nSubmissions with multiple executions: ${multipleExecutionsCount}`,
    );
    ...
    for (const userId in userStats) {
      console.log(`\nUser ${distinctIdToUsername[userId]}:`);
    ...
    console.log(
      `Total Matched Events (including multiple executions): ${matchedEvents.length}`,
    );
  } catch (error) {
