...
const findMaxActionsExecutedTimeRanges = (
  events: Event[],
  distinctId: string,
): { maxTimeRanges: number[][] } => {
  ...
  return {
    maxTimeRanges: maxTimeRanges.map((range) => [
      Math.floor(new Date(range[0]).getTime() / 1000),
      Math.floor(new Date(range[1]).getTime() / 1000),
    ]),
  };
};

const findMaxUniqueEventDaysTimeRanges = (
  events: Event[],
  distinctId: string,
): { maxTimeRanges: number[][] } => {
  const userEvents = events.filter(
  ...
  for (let i = 0; i < userEvents.length; i++) {
    ...
    if (uniqueDays > maxUniqueDays) {
      ...
      maxTimeRanges = [[startTime, endTime]];
    } else if (uniqueDays === maxUniqueDays) {
      maxTimeRanges.push([startTime, endTime]);
    }
  }

  // Convert timestamps to datetime format and compare dates
  const uniqueTimeRanges = maxTimeRanges.filter((range, index, self) => {
    const rangeDate = new Date(range[0] * 1000).toISOString().slice(0, 10);
    return (
      index ===
      self.findIndex((r) => {
        const rDate = new Date(r[0] * 1000).toISOString().slice(0, 10);
        return rDate === rangeDate;
      })
    );
  });

  return {
    maxTimeRanges: uniqueTimeRanges.map((range) => [range[0], range[1]]),
  };
};

const findFirstActionExecutedTimeRange = (
  events: Event[],
  distinctId: string,
): number[][] => {
  const userEvents = events.filter(
  ...
  const firstEvent = userEvents.reduce((earliest, current) =>
    current.properties.time < earliest.properties.time ? current : earliest,
  );

  const startTime = firstEvent.properties.time;
  const endTime = Math.floor(Date.now() / 1000); // Current time in seconds

  const timeRange = [
    [
      Math.floor(new Date(startTime * 1000).getTime() / 1000),
      Math.floor(new Date(endTime * 1000).getTime() / 1000),
    ],
  ];
...
const getInScopeAddresses = async (): Promise<string[]> => {
  try {
    const query = `
    ...
    const result = await sequelize.query<TokenInfo>(query, {
      type: QueryTypes.SELECT,
    });

    let inScopeAddresses = result.map((row) => row.address);

    const contractNameQuery = `
      SELECT LOWER(address) AS address, LOWER(contractname) AS contractname
      FROM contractname
    `;

    const contractNameResult = await sequelize.query<ContractInfo>(
      contractNameQuery,
      { type: QueryTypes.SELECT },
    );
    ...
    for (const row of contractNameResult) {
      const contractName = row.contractname;
      const address = row.address;

      if (swapKeywords.some((keyword) => contractName.includes(keyword))) {
        inScopeAddresses.push(address);
      }
    }

    // Read addresses from address.js files in the config directory
    const configDir = "/Users/dylanvu/walletapi/src/config";
    const addressFiles = getAddressFiles(configDir);

    for (const filePath of addressFiles) {
      const addressModule = await import(filePath);
      const addressData = addressModule.default;

      for (const chainId in addressData) {
        const contracts = addressData[chainId];
        for (const contractName in contracts) {
          const address = contracts[contractName];
          if (address) inScopeAddresses.push(address.toLowerCase());
        }
      }
    }

    // Remove duplicate addresses
    inScopeAddresses = [...new Set(inScopeAddresses)];

    return inScopeAddresses.filter((address) => address !== undefined);
  } catch (error) {
    console.error("Error fetching in-scope addresses:", error);
    throw error;
  }
};

// Helper function to recursively get all address.js files in a directory
const getAddressFiles = (directory: string): string[] => {
  ...
  for (const file of files) {
    ...
    if (stats.isDirectory()) {
      addressFiles.push(...getAddressFiles(filePath));
    } else if (file === "address.js") {
      addressFiles.push(filePath);
    }
  }
...
const countTransactionsBetweenDates = async (
  ...
  startDate: string,
  endDate: string,
  inScopeAddresses: string[],
) => {
  const startTimestamp = startDate;
  const endTimestamp = endDate;

  let totalTransactionCount = 0;
  let inScopeTransactionCount = 0;

  for (const address of addresses) {
    const lowercaseAddress = address.toLowerCase();
    const transactions = await sequelize.query<TransactionInfo>(
      `
    ...
    for (const transaction of transactions) {
      ...
      if (
        inScopeAddresses.includes(fromAddress) ||
        inScopeAddresses.includes(toAddress)
      ) {
        inScopeTransactionCount++;
      }
    }
  }
...
const formatDateTimeEST = (unixTimestamp: number): string => {
  return moment
    .unix(unixTimestamp)
    .tz("America/New_York")
...
const analyzeOverlap = async () => {
  ...
  const inScopeAddresses = await getInScopeAddresses();

  for (const distinctId in distinctIdToUsername) {
    const username = distinctIdToUsername[distinctId];
    const wallets = distinctIdToWallets[distinctId];

    const actionsExecutedTimeRanges = findMaxActionsExecutedTimeRanges(
      events,
      distinctId,
    );
    const frequencyTimeRanges = findMaxUniqueEventDaysTimeRanges(
      events,
      distinctId,
    );
    const totalTimeRange = findFirstActionExecutedTimeRange(events, distinctId);

    console.log(`\nUsername: ${username}`);

    console.log("Actions Executed Time Ranges:");
    for (const timeRange of actionsExecutedTimeRanges.maxTimeRanges) {
      const startDateTime = formatDateTimeEST(timeRange[0]);
      const endDateTime = formatDateTimeEST(timeRange[1]);
      console.log(`  ${startDateTime} to ${endDateTime}`);

      const { inScopeTransactionCount, totalTransactionCount } =
        await countTransactionsBetweenDates(
          wallets,
          timeRange[0].toString(),
          timeRange[1].toString(),
          inScopeAddresses,
        );
      console.log(`    In-Scope Transaction Count: ${inScopeTransactionCount}`);
      console.log(`    Total Transaction Count: ${totalTransactionCount}`);
      console.log(
        `    In-Scope Transaction Ratio: ${inScopeTransactionCount / totalTransactionCount}`,
      );
    }

    console.log("Frequency Time Ranges:");
    for (const timeRange of frequencyTimeRanges.maxTimeRanges) {
      const startDateTime = formatDateTimeEST(timeRange[0]);
      const endDateTime = formatDateTimeEST(timeRange[1]);
      console.log(`  ${startDateTime} to ${endDateTime}`);
    ...
    console.log("Total Time Range:");
    for (const timeRange of totalTimeRange) {
      const startDateTime = formatDateTimeEST(timeRange[0]);
      const endDateTime = formatDateTimeEST(timeRange[1]);
      console.log(`  ${startDateTime} to ${endDateTime}`);
