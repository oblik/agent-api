...
const findEarliestEvent = (events: Event[], distinctId: string) => {
...
const analyzeEvents = async () => {
  ...
  for (const distinctId in distinctIdToUsername) {
    const earliestEvent = findEarliestEvent(events, distinctId);

    if (earliestEvent) {
      const username = distinctIdToUsername[distinctId];
      const timestamp = formatTimestamp(earliestEvent.properties.time);
      console.log(`Username: ${username}, First Action Executed: ${timestamp}`);
    } else {
      console.log(
        `No successful action executed events found for ${distinctId}`,
      );
    }
  }
};
